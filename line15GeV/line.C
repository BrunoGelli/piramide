...
void line()
{
	...
	for (int i = 0; i < size; ++i)
	{
		...
		EnergyDist[bin]->SetTitle(Form("histogram of detected energy for the position %2.2lf m", Frente->GetBinCenter(bin)));
	}
...
void Highlight2(TVirtualPad *pad, TObject *obj, Int_t xhb, Int_t yhb)
{
	...
	if (!CanvasProj) 
	{
		...
		TCanvas *CanvasProj = new TCanvas("CanvasProj", "CanvasProj", 0, 0, 600, 600);
		CanvasProj->cd();
		EnergyDist[xhb]->Draw("");
	}
	else
