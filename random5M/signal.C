...
void signal()
{
	...
	{
		TH1D *h = new TH1D(Form("histogram of detected energy for the bin number %d", i), Form("histogram of detected energy for the bin number %d", i), 100, -5, 55); 
...
// 
	...
	// Frente->SetMaximum(600);
 	
 	TCanvas *cratio = new TCanvas();
	cratio->cd();

	TH1D *hratio = new TH1D("ratio", "ratio", 1000, 0, 2); 
	...
	TH1D *hchi2 = new TH1D("chi2", "chi2", 500, -0.5, 2); 
